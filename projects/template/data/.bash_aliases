...
sav_editor=nano
alias join_command="python -c \"import sys; print(\\\"\\\".join(sys.argv[1:]))\""
...
n() {
  # Finagling to be able to write 's, write them as 'some string' "'" 'the rest of the string'
  command=$(join_command "$@")
  eval $command
  echo "$command" >> "$notes_file"
}
