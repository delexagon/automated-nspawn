...
do_automatic() {
  if [ -f "$OUTSIDE/$defined_path/$1".sh ] ; then
    bash "$OUTSIDE/$defined_path/$1".sh
  fi
  if [ -f "$COPY/$defined_path/$1".cp ] ; then
    python3 copy_files.py "$DATA" "$THIS_CHROOT" "$COPY/$defined_path/$1".cp
  fi
  if [ -f "$THIS_PROJECT/start_$defined_path/$1" ] ; then
    cp "$THIS_PROJECT/start_$defined_path/$1" "$THIS_CHROOT/root/$1"
...
export STARTERS="$THIS_PROJECT"/start_predefined
...
if [ "$action" = init ] ; then
  ...
  cp "$THIS_PROJECT/start_$defined_path/"* "$THIS_CHROOT/root"
